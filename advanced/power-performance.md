...
启用或禁用 Intel® Speed Shift Technology。启用后处理器可更快地提升工作频率，改善系统响应速度。Auto 选项让 BIOS 自动配置此项。
...
启用或禁用 Intel® Thermal Monitor 功能（CPU 过热保护功能）。启用后，当 CPU 过热时将降低核心频率和电压。Auto 选项让 BIOS 自动配置此项。
...
启用或禁用 Enhanced Intel® Speed Step Technology（EIST，增强型英特尔节能技术）。根据 CPU 负载，Intel® EIST 技术可动态有效地降低 CPU 电压和核心频率，以降低平均功耗和发热。Auto 选项让 BIOS 自动配置此项。
...
决定是否启用 Intel® CPU 睿频加速技术。Auto 选项让 BIOS 自动配置此项。

### Active Turbo Ratios（活动睿频倍率）

说明：

允许为不同数量的活动核心设置 CPU 睿频倍率。Auto 选项根据 CPU 规格设置 CPU 睿频倍率。此项仅在 Active Turbo Ratios 设置为 Manual 时可配置。

### Turbo Power Limits（睿频功耗限制）

说明：

允许为 CPU 睿频模式设置功耗限制。当 CPU 功耗超过指定功耗限制时，CPU 将自动降低核心频率以降低功耗。Auto 选项根据 CPU 规格设置。

### DDR5 XMP Booster（DDR5 XMP 增强器）

说明：

允许在为特定内存 IC（集成电路）制造商构建的配置文件之间进行选择，以增强内存性能。该选项为 DDR5 内存平台特有。

### A.I. XMP Booster Profile（A.I. XMP 增强器配置文件）

说明：

允许在操作系统内使用 AORUS AI SNATCH 软件配置内存性能增强。
...
允许 BIOS 读取已安装 XMP 内存模块上的 SPD 数据，以在启用时增强内存性能。

Disable：禁用此功能。

Profile1：使用配置文件 1 设置。

Profile2：使用配置文件 2 设置（仅当安装了支持此功能的内存模块时可用）。

### System Memory Multiplier（系统内存倍频）

说明：

允许设置系统内存倍频。Auto 选项根据内存 SPD 数据设置内存倍频。
...
启用或禁用高带宽内存模式。Auto 选项让 BIOS 自动配置此项。
...
启用或禁用低延迟内存模式。Auto 选项让 BIOS 自动配置此项。
