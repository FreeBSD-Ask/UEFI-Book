...
该选项用于设置 Psys 平台功耗信号的最大功率阈值（Pmax），供处理器 PL4 Boost 等平台功耗管理算法作为参考值。当启用 PL4 Boost 时，处理器将使用该 Pmax 值计算可达的更高性能频率。
...
Energy Performance Gain（EPG，能效性能增益）是 Intel 平台的一项内存电源管理功能。启用后，BIOS 会根据系统工作负载动态调整内存的功耗状态，允许内存 DIMM 在有条件时进入更低的功耗状态（如主动待机 Idd3N 或主动掉电 Idd3P），从而在保证性能的前提下提升整体能效。该功能与下方的 EPG DIMM Idd3N 和 Idd3P 电流参数配合使用，用于计算内存功耗预算。
